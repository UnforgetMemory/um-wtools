/**
 * Pure JS MD5 hash implementation.
 *
 * Processes input text as UTF-8 bytes and returns a hex-encoded MD5 digest.
 * Based on the RFC 1321 specification.
 *
 * Uses a pure-JS implementation instead of crypto.subtle.digest() because
 * the Web Crypto API does not support the MD5 algorithm.
 */

// 32-bit integer addition with overflow — JS numbers are 64-bit floats so we
// manually carry the low-16 overflow into the high-16 via LSB/MSW split.
 ...
 *
 * The input is encoded as UTF-8 before hashing, matching the standard
 * behaviour of most MD5 utilities.
 */
export function md5(text: string): string {
  // Encode as UTF-8 bytes first
  const utf8Bytes = new TextEncoder().encode(text)
  // Convert UTF-8 byte array to word array (little-endian)
  const words: number[] = []
  for (let i = 0; i < utf8Bytes.length; i++) {
    words[i >> 2] |= utf8Bytes[i] << ((i % 4) * 8)
  }
  const hash = coreMd5(words, utf8Bytes.length * 8)
